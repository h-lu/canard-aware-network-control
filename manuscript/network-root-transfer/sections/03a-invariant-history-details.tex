...
polynomial \(P_*\), constants \(C_*,c_*,\Gamma_*>0\), and an integer
\(M_*\), independent of \(N\) and \(\delta\), such that the order-zero
estimate below holds for every vector field \(Q\) in the graph ball defined
in \cref{eq:invdet-graph-ball}.  At positive grade, it is required
inductively for every smooth Picard iterate whose preceding graph jets lie
in the invariant balls already constructed:
\begin{equation}
...
\(\mathscr J\) of total grade at most eleven.  The same estimate, multiplied
by the current-block difference plus preceding-block differences, holds for
two Picard iterates in the corresponding common jet fibers, where
\begin{equation}
...
\end{equation}
The ordinary flow variational equations, successively differentiated
through grade eleven in the block order used below, then prove
\cref{eq:invdet-tame-flow}.  The state derivative of the perturbation in
...
\end{equation}
This is also the grade-zero difference case of
\cref{eq:invdet-tame-flow}.  Combining it with the Lipschitz bound for
...
There are \(C,c>0\) and an integer \(M\), independent of \(N\) and
\(\delta\), such that, for \(\ell=1,2\), the reduced coefficients after
conversion back to the \((X,Y)\)-coordinates satisfy
\begin{equation}
 \left|D_u^a\partial_\nu^i\partial_\zeta^j
...
\]
After increasing \(C,c,M\), the same bound holds for the stable
...
We record the local tame bound used in the main proof.  Split the stable
integral after every derivative at
\(L\log(1/\delta)\).  By choosing \(L\) after the finite jet grade, its tail
is bounded by
\begin{equation}
 C\mathfrak B_\delta
 \{1+L\log(1/\delta)\}^{M_*}
...
\(\delta L\log(1/\delta)=o(1)\).  Let \(L_J\) be the number of ordered
blocks and choose fixed buffers
\[
 0=C_0<C_1<\cdots<C_{L_J},\qquad
 C_{\ell+1}-C_\ell>2\Theta_*+2.
\]
At block \(\ell\), take the weighted supremum on the core interval and use
the successive fixed buffers for the already known lower blocks.  Every
current or delayed flow evaluation of a lower block, and every finite
convolution shift, then lands in its assigned outer domain.  The weights
\(W_{c,m}(s)=e^{c|s|}\langle s\rangle^m\) obey
\[
 W_{c,m}(s+t)\leq C_TW_{c,m}(s),\qquad |t|\leq2\Theta_*+2,
\]
with \(C_T\) independent of \(s,N,\delta\).

For points a fixed buffer away from \(\pm S_\delta\), all the sampled
points lie in the uncut physical hull; the polynomial chart and its
singular variational equations give \(CW_{c,m}(s)\).  In the complementary
endpoint strips, \(R_\delta-|s|\) is bounded, so the global tame majorant
\(P(R_\delta)e^{cR_\delta}\) is bounded by \(CW_{c',m'}(s)\).
...
appropriate local weight, the explicit highest-block formula
\cref{eq:invdet-highest-block-form} therefore becomes
\begin{equation}
 \|\mathbf J_\ell Z\|_{\ell;W}
 \leq B_{\ell;W}
   +\eta_\delta R_{\ell,\delta}^{\rm glob}
   +\sum_{k<\ell}C_{\ell k}\|\mathbf J_k Z\|_{\ell+1;W},          \label{eq:invdet-local-block-recurrence}
\end{equation}
where \(B_{\ell;W}\) and \(C_{\ell k}\) are independent of \(N,\delta\),
and
\[
 \eta_\delta R_{\ell,\delta}^{\rm glob}
...
The tail
\cref{eq:invdet-convolution-tail} is absorbed in \(B_{\ell;W}\).
Induction in \(\ell\), starting with the order-zero graph bound, gives one
uniform radius for every required local weighted block.  Applying this to
...
coordinate loss, proves
\begin{equation}
 |D_u^a\partial_\nu^i\partial_\zeta^j
       \mathcal R_{3,N}(\gamma_0(s)+\xi)|
 \leq C\langle s\rangle^Me^{c|s|},
 \qquad a\leq3,\quad i,j\leq2,                     \label{eq:invdet-local-remainder}
\end{equation}
on a nested retained tube.  This also makes explicit why a global
\(C_b^3\) remainder estimate independent of \(\delta\) is neither claimed
nor needed.

The same split gives the fixed-compact graph size asserted in
\cref{lem:shared-graph-trace}.  On a fixed compact \(K\subset U\), the
finite part of the stable integrand remains in one fixed physical
neighborhood and is bounded independently of \(\delta\); its integral is
therefore \(O(\delta)\).  The complementary tail is bounded by
\(\delta A_\delta O(\delta^6)=o(\delta)\).  Consequently
\begin{equation}
 \sup_{N,\nu,\zeta}\sup_{u\in K}
       \|H_{N,\delta,\nu,\zeta}(u)\|_N
 \leq C_K\delta.                                      \label{eq:invdet-fixed-compact-size}
\end{equation}
...
\end{equation}
Consequently \(Z_{1,N}\) and \(Z_{2,N}\) depend only on the physical data
on the declared depth-one and depth-two hulls, respectively.  The full
fixed point still uses the untruncated integral in
...
\cref{sec:model-results}, use the pointwise inverse fold chart together
with the time reparametrization
\(\vartheta=\delta\theta_{\rm phys}\).  This gives a continuous injective
map from the slow history interval \([-\Theta_*,0]\) onto the original
history interval \([-\Theta_*/\delta,0]\).  Consequently the embedding and
history-equality statements above are statements about the full original
RFDE histories, not only about their slow-coordinate representatives.
