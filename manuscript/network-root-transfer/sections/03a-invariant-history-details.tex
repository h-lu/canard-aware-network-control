...
The preparation in \cref{def:admissible-preparation} contains a tame-flow
...
\cref{def:admissible-preparation}, and denote the resulting globally bounded
maps by
\begin{align}
...
the depth-two hull in \cref{def:admissible-preparation} is contained in
\[
...
preparation already required in \cref{def:admissible-preparation}, is a
...
\cref{def:admissible-preparation} includes the explicit singular cutoff and
...
\(\delta_0\) once more if necessary.
...
the terms linear in \(V\) in the variational systems in
...
geometrically.  \Cref{eq:invdet-highest-block-difference} gives
\begin{equation}
...
\begin{proposition}[Graph-only coefficient envelopes]
...
fixed tube data and on the common model and graph-preparation data from the
first part of \cref{def:admissible-preparation}, but independent of \(N\),
...
\(|s|\leq S_\delta+1\), \(\nu\in I_\nu\),
\(|\zeta|\leq\zeta_*\), and either
\begin{equation}
 |\xi|\leq C_0\delta e^{c_0|s|}\langle s\rangle^{M_0},          \label{eq:invdet-trace-input-tube}
\end{equation}
or the evaluation point is
\begin{equation}
 \gamma_0(s)+\xi=\Gamma(s,d),\qquad |d|\leq\delta^\kappa.      \label{eq:invdet-retained-input-tube}
\end{equation}
After increasing the output constants \(C,c,M\), without changing these
input sets, the same bound holds for the stable
coefficients \(H_{1,N},H_{2,N}\).  Moreover,
\begin{equation}
 \partial_\zeta q_{1,N}=0,\qquad
 \partial_{\nu\nu}q_{1,N}=0.                         \label{eq:invdet-q1-zeta-zero}
\end{equation}
\end{proposition}

Only \(\mathfrak P_{\rm gr}\) enters this proposition.  In particular, its
proof is logically prior to the construction of the planar trace completion
\(\mathfrak P_{\rm pl}\).
...
\Cref{eq:invdet-q1-q2-formulas,eq:invdet-H1-H2-formulas} show
...
For \(|s|\leq S_\delta+1\), all points generated from either
\cref{eq:invdet-trace-input-tube} or
\cref{eq:invdet-retained-input-tube} belong to the continuous depth-two
uncut hull in \cref{def:admissible-preparation}, after decreasing
\(\delta_0\).  For the second alternative this is exactly how that hull was
defined; for the first, use
\(\delta e^{c_0S_\delta}\langle S_\delta\rangle^{M_0}
=o(\delta^\kappa)\).  The raw
...
The formula for \(\widehat q_{1,N}\) in
\cref{eq:invdet-q1-q2-formulas} is affine in \(\nu\) (indeed its resource
component is exactly \(\nu\)); the base flow is \(\nu\)-independent.
Consequently its second \(\nu\)-derivative also vanishes, and the
parameter-independent pullback preserves this identity.
\end{proof}
...
\]
The same estimate, after one further fixed enlargement of the output
weight, holds on the entire retained set
\begin{equation}
 |s|\le S_\delta+1,\qquad
 \gamma_0(s)+\xi=\Gamma(s,d),\qquad |d|\le\delta^\kappa.       \label{eq:invdet-retained-remainder-tube}
\end{equation}
To see this without importing the planar completion, rerun the finite
local-block induction with a second nested family whose initial normal
radius is \(\delta^\kappa\):
\begin{equation}
 \mathcal T^{\rm ret}_{j,\delta}
 :=\left\{\Gamma(\chi,d):
   |\chi|\le S_\delta+1+B_j^{\rm ret},\quad
   |d|\le K_j^{\rm ret}\delta^\kappa
      e^{c_j^{\rm ret}|\chi|}\langle\chi\rangle^{m_j^{\rm ret}}
   \right\}.                                        \label{eq:invdet-retained-nested-tubes}
\end{equation}
Choose the finitely many buffers and weights successively in the jet-block
order so that each current evaluation, fixed-delay backtrack, and finite
flow variation used at one block maps into the next tube.  There may be
more than two delay backtracks in the third and higher jets; their number is
finite because the prolonged jet list is finite.  Uniform singular-flow
bounds and
\[
 \delta^\kappa e^{O(R_\delta)}=o(1),\qquad
 \delta A_\delta=o(\delta^\kappa)
\]
show that all these finite pieces, including the displacement caused by the
full graph, remain in the fixed prepared \(b_0\)-neighborhood and stable
saturation ball.  Samples belonging to the declared depth-two agreement
hull use the physical polynomial field; all further finite samples use the
fixed graph preparation and its tame bounds.

On the inner portions of the buffered tubes the resulting physical or
prepared polynomial factors are controlled by a fixed output weight.  On
the complementary portions one has
\(\bigl||\chi|-S_\delta\bigr|=O(1)\), with the constant taken from the
finite buffer list, so the global tame majorant
\(P(R_\delta)e^{cR_\delta}\) is absorbed after enlarging the factor
\(e^{c|\chi|}\langle\chi\rangle^M\).  The infinite convolution tail is
unchanged and remains \(O(\delta^6A_\delta)\).  Thus the same recurrence
\cref{eq:invdet-local-block-recurrence} applies to the retained nested
tubes, with constants independent of \(N,\delta\), and yields
\cref{eq:invdet-local-remainder} on
\cref{eq:invdet-retained-remainder-tube}.
...
counterpart.  \Cref{eq:invdet-semiconjugacy} is therefore an exact
...
preparation satisfying \cref{def:admissible-preparation}, including its
...
\cref{eq:shared-trace-jets} are proved separately in the finite-dimensional
trace argument beginning with \cref{eq:shared-trace-operator}; they are not
part of the present history-graph lemma.
