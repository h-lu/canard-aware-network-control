...
\begin{equation}
 ...
 R_\delta=c_{\rm hl}\delta^6,                         \label{eq:anchor-half-line-radius}
\end{equation}
...
\begin{equation}
 ...
 \le\frac{c_{\rm hl}}{12C_G}
       \delta^{M_G}R_\delta .                         \label{eq:anchor-seed-residual-smallness}
\end{equation}
...
\(\|\bar z\|_{\mathscr Z_\beta}\le R_\delta/3\).
Consequently
\begin{equation}
...
radius-\(R_\delta\) nonlinear chart and its contraction unchanged.  Thus
...
\begin{equation}
 \operatorname{Lip}_{\mathscr Z_\beta\to\mathscr F_\beta}
       (\mathcal N_-;R_\delta)
 \le C\delta^{-2}R_\delta,\qquad
 \|\widetilde{\mathbb G}_-\|
       \operatorname{Lip}_z(\mathcal N_-;R_\delta)
...
\begin{equation}
 ...
 =\{\Phi_{\rm ent}(\psi,F^{-,\rm ent}_{N,\delta,p}(\psi);p):
                         \|\psi\|<R_\delta\}.         \label{eq:anchor-intrinsic-entry-sheet}
\end{equation}
...
\end{equation}
whose selected terminal graph stays in the radius-\(R_\delta\) tube.
...
\cref{eq:anchor-chart-center-fixed-point} inside the \(R_\delta/12\)
ball.  Moreover,
\[
 \|\mathbb G_\Gamma^{\rm ent}\|
 \operatorname{Lip}_{\mathscr Z_\beta\to\mathscr F_\beta}
       (\mathcal N_\Gamma;R_\delta)
...
most \(C\delta^{-1}R_\delta=C\delta^5\), whereas the reference speed has
...
\(O(R_\delta/\delta)\).  In the normalized source norm the corresponding
Green perturbation number is at most
\(C\delta^{-M_G-2}R_\delta=C\delta^2\).  Dichotomy roughness therefore
...
\(R_\delta/3\), and the graph correction for
\(\|\psi\|\le R_\delta\) is at most \(CR_\delta\).  The phase/fading norm
...
\cref{eq:anchor-intrinsic-entry-sheet} is exactly the set of histories on
\(\Sigma_-^{\rm ent}\) whose forward orbits remain in the declared channel
and converge to \(E_N^-\).  The physical-speed central family and the
three reserved delay buffers define the first-hit map
\(\mathcal P_{0\to{\rm ent},p}\) in
\cref{eq:anchor-forward-central-entry-holonomy}.  It evolves a compatible
history on \(\Sigma_0\) forward until its first regular hit of
\(\Sigma_-^{\rm ent}\); no backward continuation is part of its definition.
The strong event theorem makes this map \(C^2\) in state and \(C^1\) in the
fixed parameter chart on the declared tube.
...
\]
a \(C^2\)-in-state, \(C^1\)-in-parameter map into
...
\cref{eq:anchor-first-integral-local-invertibility}.  The parameterized
Banach inverse-function theorem therefore gives the exact chart
\cref{eq:anchor-central-history-chart,eq:anchor-central-chart-identities}.

Compose the exact entry membership function
\cref{eq:anchor-entry-membership-function} with this forward hit and the
central chart.  This is precisely \(\mathfrak M_{0,p}\) in
\cref{eq:anchor-forward-preimage-membership}; its zero set is the forward
preimage of the intrinsic entry sheet.  We next verify its scalar
transversality without identifying that sheet with a finite-core terminal
row.

At a zero of \(\mathfrak M_{0,p}\), put
\begin{align}
 k_0^u&=D_u\mathfrak C_{0,p}(\psi,u)1,\notag\\
 \phi_{\rm ent}
   &=\mathcal P_{0\to{\rm ent},p}
        (\mathfrak C_{0,p}(\psi,u)),
 & (\psi_{\rm ent},u_{\rm ent})
   &=\Pi_{\rm ent,p}\phi_{\rm ent}.                \label{eq:anchor-central-entry-column-data}
\end{align}
The exact entry splitting gives the unique decomposition
\begin{equation}
 D\mathcal P_{0\to{\rm ent},p}k_0^u
 =k_{\rm ent}^s+a_u\,
   D_u\Phi_{\rm ent}
      (\psi_{\rm ent},F^{-,\rm ent}(\psi_{\rm ent});p)1,
 \qquad
 k_{\rm ent}^s\in T_{\phi_{\rm ent}}\mathcal S_-^{\rm ent}(p).
                                                               \label{eq:anchor-central-entry-column-splitting}
\end{equation}
Applying \(D\mathfrak m_{{\rm ent},p}=\lambda_{\rm ent}\) and using
\cref{eq:anchor-dual-row-pullback} gives the exact typed identity
\begin{equation}
 \partial_u\mathfrak M_{0,p}(\psi,u)
 =\lambda_{\rm ent}(\psi_{\rm ent};p)
      D\mathcal P_{0\to{\rm ent},p}k_0^u
 =a_u.                                                \label{eq:anchor-forward-column-coefficient-identity}
\end{equation}
We prove that this coefficient is nonzero without comparing two long
exponential products.  Use first the exact resource-gauge quotient of
\cref{prop:exact-resource-gauge-quotient}.  The generated segment through
the zero determines, in the fixed original-endpoint chart, its actual
entry/old-history gauge \(g_{\phi,p}\) from
\cref{prop:parameter-coherent-physical-hit-family}.  After the already
permitted common shrink of the parameter box and the declared tube,
\begin{equation}
 \mathbf p_\phi:=(\nu,\eta,g_{\phi,p})
       \in B_{\rm oe}(p^\circ,r_{\rm sp});             \label{eq:anchor-actual-oe-member}
\end{equation}
no assertion that \(g_{\phi,p}=0\) is made.  The anchored and original
equations agree on this segment; the gauge only records which member of the
original-endpoint chart represents the generated history.

Let \(s_b\) be the time of the first generated cut
\(r=-\rho_{b,\delta}\), let
\(s_{\rm hit}>s_b\) be the fold time of the subsequent entry hit, let
\(Y_p(s)=D\mathcal P_{0\to s,p}k_0^u\) be the event-normal variational
column, and let \(\lambda_p^u(s)\) be the dual row of the exact
one-dimensional unstable bundle along the zero orbit, normalized on an
oriented generator \(h_p^u(s)\).  This bundle is the
\(C\delta^{-2}R_\delta\)-roughness continuation of the chart-center
half-line dichotomy to the stable-sheet orbit through the zero.  On the
overlap with the generated core
this is the bundle of
\cref{eq:oe-family-core-bundles}; on the outer continuation it is the
bundle in \cref{eq:anchor-annulus-Green-kernels}.  Uniqueness of a
one-dimensional dichotomy bundle identifies the two on their common cut.
Put
\begin{equation}
 \gamma_p(s)=\lambda_p^u(s)Y_p(s),\qquad
 Y_p(s)=Y_p^s(s)+\gamma_p(s)h_p^u(s).              \label{eq:anchor-central-column-scalar-splitting}
\end{equation}
The stable and unstable summands are invariant.  Consequently there is a
scalar cocycle \(\chi_p(s,t)>0\), with the orientation fixed at \(s_b\), such
that
\begin{equation}
 \gamma_p(s)=\chi_p(s,s_b)\gamma_p(s_b),\qquad
 \chi_p(s,t)=
   \exp\!\left\{\int_t^s\varkappa_p^u(\zeta)\,\dd\zeta\right\}.
                                                               \label{eq:anchor-central-column-scalar-cocycle}
\end{equation}
On the fixed overlap the integral is bounded below, and on every outer
repelling subannulus
\(\varkappa_p^u\ge c/\delta\).  Hence
\begin{equation}
 \chi_p(s_{\rm hit},s_b)\ge c_\chi>0                 \label{eq:anchor-central-column-cocycle-lower-bound}
\end{equation}
uniformly; the factor may be exponentially large, but it is never compared
additively with an \(O(\delta)\) remainder.

It remains only to estimate the coefficient at the first cut.  Let
\(Y_0\) be the singular planar variational column generated by a unit
\(Y\)-variation at \(\Sigma_0\), and let \(\gamma_0^u(s_b)\) be its
coefficient on the oriented repelling mode.  Since the central chart uses
\(u=c_{\mathscr H}\mathscr H_\alpha(0,Y)\),
\begin{equation}
 \gamma_0^u(s_b)
 =\mathcal A_{0\to b}^{u,(0)}
   \bigl[D_Y\{c_{\mathscr H}\mathscr H_\alpha(0,Y)\}\bigr]^{-1},
 \qquad |\gamma_0^u(s_b)|\ge c_b^u>0.                \label{eq:anchor-first-cut-singular-coefficient}
\end{equation}
The last inequality is the explicit repelling-mode coefficient of the
planar fold fundamental solution and uses
\cref{eq:anchor-first-integral-local-invertibility}.

For completeness, factor the common singular unstable scalar fundamental
from the exact and singular columns on the interval from \(\Sigma_0\) to
\(s_b\).  Variation of constants for the remaining projective coefficient
gives
\begin{equation}
 \left|\frac{\gamma_p(s_b)}{\gamma_0^u(s_b)}-1\right|
 \le C\left\{
   \delta e^{cS_\delta}\langle S_\delta\rangle^M
   +\eta_{{\rm core},\mathbf p_\phi}
   +\eta_{{\rm fr},\mathbf p_\phi}
   +\delta^{-2}R_\delta\right\}
 =: \varepsilon_b(\delta)=o(1).                   \label{eq:anchor-first-cut-projective-comparison}
\end{equation}
Indeed, the first term is the integral of the central trace-coefficient
difference after the singular scalar has been factored out; the next two
are precisely the moving-frame and generated-core projective Green
numbers in
\cref{eq:oe-family-core-mixed-green-bound,eq:oe-family-core-dichotomy-bounds};
the last is the roughness number for the radius-\(R_\delta\) stable-sheet
orbit.  Exponential separation makes the stable-to-unstable projective
kernel integrable, so no factor equal to the total unstable action occurs
in this estimate.  This is a scalar variation-of-constants estimate for the
coefficient at \(s_b\), not a relative comparison of the complete
fold-to-entry amplitudes.

After decreasing \(\delta_0\),
\(|\gamma_p(s_b)|\ge c_b^u/2\).  At the entry graph point the tangent space
of the intrinsic nonlinear stable sheet is exactly the set of variational
data converging at \(+\infty\); hence its normalized annihilator is
\(\lambda_{\rm ent}\).  Equations
\cref{eq:anchor-forward-column-coefficient-identity,eq:anchor-central-column-scalar-cocycle}
therefore give, exactly,
\begin{equation}
 |a_u|=|\gamma_p(s_{\rm hit})|
 =\chi_p(s_{\rm hit},s_b)|\gamma_p(s_b)|
 \ge \tfrac12c_\chi c_b^u.                         \label{eq:anchor-direct-column-transversality-ledger}
\end{equation}
Shrinking the state chart preserves half this margin and proves
\cref{eq:anchor-forward-membership-transversality}.  The argument uses
only the exact intrinsic dichotomy and its one-dimensional scalar cocycle;
it neither identifies the finite physical-speed row with the nonlinear
stable sheet nor hides an exponentially scaled column in an absolute
\(O(\delta)\) comparison.

The parameterized implicit-function theorem now gives the central graph
\(u=F^{-,0}_{N,\delta,p}(\psi)\), proving
\cref{eq:anchor-stable-sheet-graph,eq:anchor-central-graph-preimage-identity}.
It is \(C^2\) in state and \(C^1\) in parameters.  Differentiating the
composition gives the forward covector pullback
...
\[
 Q^{\sigma,\rm phys}
 =z^{\sigma,\rm phys\,\prime}-q_0(z^{\sigma,\rm phys})
...
Put \(R=S_\delta+B\), and let \(z^{a,\rm phys}\) and \(z^{r,\rm phys}\)
be the planar core traces of the exact paired histories through
\(A^0\) and \(B^0\).  Define
\begin{align}
 \mathcal B_{\rm out}:={}&
 c_{\mathscr H}\{\mathscr H_\alpha(z^{a,\rm phys}(-R))
       -\mathscr H_\alpha(z^{r,\rm phys}(R))\}\notag\\
 &+c_{\mathscr H}\int_{-R}^{-S_\delta}
   \nabla\mathscr H_\alpha(z^{a,\rm phys})^T
       Q^{a,\rm phys}\,\dd s
 +c_{\mathscr H}\int_{S_\delta}^{R}
   \nabla\mathscr H_\alpha(z^{r,\rm phys})^T
       Q^{r,\rm phys}\,\dd s .                     \label{eq:anchor-outer-endpoint-strip}
\end{align}
Here \(Q^{\sigma,\rm phys}=z^{\sigma,\rm phys\,\prime}
-q_0(z^{\sigma,\rm phys})\) is the exact planar perturbation of the actual
RFDE trace.  Define the linear structural profile by
\[
 a^{\rm phys}_{2,N}(s)[\eta]
 =-e^{-s^2/2}s^2\Lambda_N(\eta).
\]
It is oriented so that
\[
 \int_{\mathbb R}a^{\rm phys}_{2,N}(s)[\eta]\,\dd s
...
\[
 \mathcal I^\sigma(s)
 =c_{\mathscr H}\nabla\mathscr H_\alpha(z^{\sigma,\rm phys}(s))^T
       Q^{\sigma,\rm phys}(s).
\]
...
\begin{align}
 \mathcal I^\sigma(s)
  -\delta a^{\rm phys}_{1,N}(s,\nu)
  -\delta^2a^{\rm phys}_{2,N}(s)[\eta]
...
\begin{align}
 ...
 c_{\mathscr H}\nabla\mathscr H_\alpha(\gamma_0)^T
   ...
   &=\delta^2a^{\rm phys}_{2,N}(s)[\eta],            \label{eq:anchor-endpoint-coefficient-table}\\
 \mathcal E_{\rm tr}^\sigma
   &:=c_{\mathscr H}
     \{\nabla\mathscr H_\alpha(z^{\sigma,\rm phys})^T
          q_{1,N}(z^{\sigma,\rm phys},\nu)
       -\nabla\mathscr H_\alpha(\gamma_0)^T
          q_{1,N}(\gamma_0,\nu)\},\notag\\
 \mathcal E_{\rm str}^\sigma
   &:=c_{\mathscr H}\nabla\mathscr H_\alpha
        (z^{\sigma,\rm phys})^T
       \{q_{2,N}(z^{\sigma,\rm phys},\nu,\eta)
          -D_\eta q_{2,N}(\gamma_0,\nu,0)[\eta]\},\notag\\
 \mathcal E_{\rm jet}^\sigma
   &:=c_{\mathscr H}\nabla\mathscr H_\alpha
        (z^{\sigma,\rm phys})^T\delta^3\mathcal R_{3,N}.
\end{align}
...
\begin{equation}
 G^{\rm phys}=u_A-F^{-,0}(\psi_A)                     \label{eq:anchor-gap-exact-coordinate-identity}
\end{equation}
with no Taylor multiplier.  Since \(\mathfrak C_{0,p}\) is injective,
\(G^{\rm phys}=0\) if and only if \(A^0=B^0\), which is exactly membership
...
\begin{align}
 ...
 &+c_{\mathscr H}\int_{0}^{S_\delta}
   \nabla\mathscr H_\alpha(z^{r,\rm phys})^T
      Q^{r,\rm phys}\,\dd s .                     \label{eq:anchor-exact-central-outer-split}
\end{align}
...
The tracking-ball estimate in
\cref{lem:anchor-annulus-Green-graphs} gives the \(C\delta^2\) outer
history bound.  Crossing a fixed \(r\)-subannulus under
...
future-asymptotic sheet.  The restricted event holonomies between two regular
cuts are \(C^1\) bijections; their inverse means recutting the same already
constructed complete or future-asymptotic solutions, not inverting the
ambient RFDE semiflow.  They carry the incoming point and stable sheet
together, so their scalar defining functions differ by a nowhere-zero
factor.  At a zero, multiplication by that factor multiplies the differential
and leaves its line unchanged.  A parameter change acts by the ordinary
covector pullback.

For intermediate sections write the total forward holonomy as
\(P=P_j\circ\cdots\circ P_1\).  The chain rule gives
\(DP=DP_j\cdots DP_1\).  Inserting or deleting a factor and its adjoining
section identification leaves the transported stable-sheet membership row
unchanged; this is the complete-history transition-jet composition law.
