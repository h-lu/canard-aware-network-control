...
\cref{prop:fold-invariant-histories,thm:rw-local-fold-matching}.  We first
...
\end{align}
Evaluation of an already supplied history at these \(m+2\) slots is a
bounded linear map, of norm at most \(m+2\) for the sum norm on the tuple,
independently of \(N\).  The dependence of \(\mathcal E_{Q,H}\) on
\((Q,H)\) additionally uses the flow estimates below.

\subsection{The invariant graph and its derivatives}

On the closed set of bounded globally Lipschitz planar fields and bounded
globally Lipschitz maps into \(E_N\) satisfying
\cref{eq:supp-graph-ball}, use the uniform product metric and define
\begin{align}
 \mathcal T_Q(Q,H)(u)
...
\end{equation}
For pairs in this ball, the global Lipschitz bound gives a unique complete
flow and the corresponding uniform flow-difference estimates.  For smooth
Picard iterates, the flow variational equations and
\eqref{eq:supp-singular-lipschitz} additionally give, for each member
\(\mathscr J\) of the finite derivative list used below,
\begin{equation}
...
\end{equation}
The constants for the higher derivatives are taken on the derivative balls
constructed successively below; no high-order differentiability is asserted
for an arbitrary Lipschitz candidate.  The corresponding difference estimate
has the same right-hand side times the differences of the current and
preceding derivative blocks.  Combining
this estimate with
...
\end{align}
for \(a\le3\), \(i\le2\), and \(-R\le s\le R\).  The extension operator and all
...
which are the estimates asserted in \cref{thm:rw-local-fold-matching}.
\end{proof}
