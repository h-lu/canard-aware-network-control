...
We use fold time \(s=\delta t\), so the delay translations are
\(s\mapsto s-\theta_k\).  Characteristic roots in the hypothesis below are
denoted by \(\lambda_t\) and refer to the original time \(t\).
...
\end{equation}
Item~\textup{(i)} of \(\mathrm{(H_{\rm glob})}\) below assumes that this
matrix has a simple real eigenvalue \(\lambda_{c,N}(r)\) near zero and
constants \(0<c_f<C_f\), independent of \(N\) and the parameter box, such
that
\[
 \operatorname{sgn}\lambda_{c,N}(r)=-\operatorname{sgn}r,
 \qquad c_f|r|\le|\lambda_{c,N}(r)|\le C_f|r|.
\]
Under that hypothesis, on either regular branch set
\begin{equation}
...
\(\mathcal T_N\).
Every statement below that uses a global orbit segment, a characteristic-root
count, or an invariant manifold associated with \(Z_N^\pm\) is conditional on
\(\mathrm{(H_{\rm glob})}\) in
\cref{hyp:supp-outer-manifolds}.
...
\item On the two regular reference intervals, the frozen voltage matrix
\(\mathcal J_N^{\rm fv}(r)\) has a simple real eigenvalue
\[
 \lambda_{c,N}(r)=-2\alpha r+O(r^2),
\]
with the sign stated above; the remainder and the complementary exponential
dichotomy are uniform in \(N\) and in the parameter box.  For every fixed
\(0<\delta<\delta_0\), the characteristic equation at \(Z_N^\pm\) has no
root on the imaginary axis.  Apart from the slow root below, the relevant
stable and unstable roots have a common positive separation from that axis.
At \(Z_N^+\) the only unstable root is the simple slow root
\begin{equation}
 \lambda_{s,\pm}^{t}
...
\(D_N^{\rm fin}\) in \cref{thm:rw-local-fold-matching}, intersected with
...
All spectral and global properties in \(\mathrm{(H_{\rm glob})}\) are
hypotheses of the conditional application; none is deduced here from the
displayed network assumptions.  In particular, local hyperbolicity alone
does not supply the global orbit segments, their intersection, or the
...
they are not derived here from the local fold calculation.  For
...
repelling trace used in \cref{thm:rw-local-fold-matching}.  There is a
...
finite-interval function in \cref{thm:rw-local-fold-matching}.  For every
