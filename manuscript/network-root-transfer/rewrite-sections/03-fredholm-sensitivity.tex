\section{Fredholm reduction and sensitivity of a defining function}
...
\begin{proposition}[A norm-optimal right inverse for the first delay moment]
...
For a fixed topology or another linear matrix restriction, its replacement is
the following elementary range criterion.

\begin{proposition}[First-moment range under prescribed matrix patterns]
\label[proposition]{prop:fredholm-structured-delay-range}
Let \(k_-\) and \(k_+\) index two delay values with
\(\theta_{k_-}<\theta_{k_+}\), and let
\[
 \mathcal U_N\subset
 \{R\in\mathbb R^{N\times N}:\pi_N^TR=0\}
\]
be a linear space.  On the restricted tuples
\[
 R_{k_-}=R,\qquad R_{k_+}=-R,
 \qquad R\in\mathcal U_N,
\]
with all other components zero, one has
\begin{equation}
 \operatorname{Ran}\mathsf S_N
 =-\frac{K(\theta_{k_+}-\theta_{k_-})}{2\alpha}
       \{R\mathbf1:R\in\mathcal U_N\}.               \label{eq:fredholm-structured-range}
\end{equation}
Hence this restriction is onto \(E_N\) precisely when the evaluation map
\begin{equation}
 \mathsf e_N:\mathcal U_N\longrightarrow E_N,
 \qquad \mathsf e_N(R)=R\mathbf1                     \label{eq:fredholm-pattern-evaluation}
\end{equation}
is onto.  It is zero whenever the allowed matrices have zero row sums.

If \(P_N\) is irreducible, the space
\begin{equation}
 \mathcal U_N^{\rm gen}
 =\{(P_N-\Id)\diag(d):d\in E_N\}                    \label{eq:fredholm-generator-pattern}
\end{equation}
has an onto evaluation map and is supported on the entries of
\(P_N-\Id\).  If the two base delay layers are nonnegative and strictly
positive on that support, then sufficiently small scalar multiples of any
direction in \(\mathcal U_N^{\rm gen}\) remain nonnegative and preserve the
existing support of both layers.  No dimension-uniform bound for a right inverse of
\(\mathsf e_N\) is asserted without a corresponding uniform hypothesis on
the prescribed pattern.

If \(\tau(P_N)\le1-\gamma\), define
\begin{equation}
 \mathsf J_Ny=(P_N-\Id)\diag(d_y),
 \qquad
 d_y=\{(P_N-\Id)|_{E_N}\}^{-1}y.                    \label{eq:fredholm-generator-right-inverse}
\end{equation}
Then \(\mathsf J_Ny\,\mathbf1=y\) and
\begin{equation}
 \norm{\mathsf J_N}\le\frac{2(2-\gamma)}{\gamma}.
 \label{eq:fredholm-generator-right-inverse-bound}
\end{equation}
The resulting right inverse of the structured first-moment map has norm at
most
\begin{equation}
 \frac{8\alpha(2-\gamma)}
      {|K|(\theta_{k_+}-\theta_{k_-})\gamma}.         \label{eq:fredholm-structured-source-inverse-bound}
\end{equation}
For the base layers
\(B_{k_-}=B_{k_+}=(P_N+\Id)/2\), the two perturbed layers associated with
\(R=(P_N-\Id)\diag(d)\) preserve nonnegativity and the common support when
\(|\zeta|\norm{d}_\infty<1/2\).
\end{proposition}
...
\cref{eq:fredholm-structured-range} follows from
...
\cref{eq:fredholm-generator-right-inverse-bound}.  Multiplying
\(\mathsf J_N\) by \(-2\alpha/
\{K(\theta_{k_+}-\theta_{k_-})\}\) in layer \(k_-\), and by the opposite
coefficient in layer \(k_+\), gives
\cref{eq:fredholm-structured-source-inverse-bound}.  Finally, every off-diagonal
...
\begin{proposition}[Uniform mixing and degeneration on a local cycle]
\label[proposition]{prop:fredholm-growing-families}
The two network families in \cref{prop:rw-growing-families} satisfy
\cref{eq:rw-growing-response,eq:rw-cycle-degeneration}.
\end{proposition}
