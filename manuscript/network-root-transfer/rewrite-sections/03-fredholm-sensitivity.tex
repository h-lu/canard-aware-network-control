\section{Fredholm reduction and a conditional heteroclinic-sensitivity theorem}
...
theorem converts the Fredholm coefficient into the derivative of the
...
Retain the network data, projections, norm, and constraint space
\(\mathfrak T_N^{\rm red}\) from
\cref{eq:rw-markov-assumptions,eq:rw-collective-transverse,%
eq:rw-network-norm,eq:rw-structural-parameter-space}.  When differentiating with
respect to the delayed-coupling matrices, write a tangent direction as
\(R=(R_0,\ldots,R_m)\in\mathfrak T_N^{\rm red}\).  Thus the \(R_k\) below
are tangent matrices satisfying the same constraints as the components
\(E_k\) in \cref{eq:rw-structural-parameter-space}; they are not a second
...
\begin{proposition}[Inversion of the first delay moment]
\label[proposition]{prop:fredholm-two-delay-inverse}
Merge coincident delay atoms and put
...
\[
 \mathsf S_N:\mathfrak T_N^{\rm red}\longrightarrow E_N
\]
...
If the merged support consists of one location, then
\(\mathsf S_N=0\) on \(\mathfrak T_N^{\rm red}\).  Thus two distinct
locations are necessary and sufficient for surjectivity of this
first-moment map on \(\mathfrak T_N^{\rm red}\).  No assertion
...
\[
 ...
 \le\frac{|K|\Delta_\theta}{4\alpha}
      \norm{R}_{\mathfrak T_N}.
\]

The tuple in \cref{eq:fredholm-delay-right-inverse} has zero layer sum and
...
\begin{theorem}[Collective--transverse Fredholm reduction]
...
there is a unique scalar functional \(\Lambda_N\in\mathcal T_N^*\) such
...
been constructed on a common phase section.  Denote the incoming hit by
...
\subsection{The network-transverse functional and the quadratic coefficients}

The connection derivative determines only the transverse functional that
has entered the one-dimensional Fredholm equation.  It does not determine
the coupling matrix \(P_N\) or an arbitrary history state.

\begin{proposition}[Recovery of the transverse linear functional]
...
the corresponding transverse functional is
...
\(R(d)\in\mathfrak T_N^{\rm red}\) be the tangent direction with
