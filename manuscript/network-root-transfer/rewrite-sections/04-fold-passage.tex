...
For the three-node parameters in the introduction,
\cref{fig:three-node-finite-section} illustrates the preceding derivative
estimate with a one-orbit finite-section calculation.  In panel~\textbf{(a)},
the vertical coordinate is a scalar section mismatch; only the horizontal
locations of its marked zero crossings are being compared.  Panel~\textbf{(b)}
centers the two perturbed zeros, converts from \(\nu\) to
\(\mu=\delta^2\nu\), and divides by \(\delta^3\), so the dashed line is the
Fredholm coefficient predicted by \eqref{eq:fold-return-pairing}.  Because the
incoming history is prescribed rather than taken from the invariant history
graph, the displayed zeros are numerical diagnostics and are not zeros of
\(D_3^{\rm fin}\).
