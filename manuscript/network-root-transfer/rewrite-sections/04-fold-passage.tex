...
the vertical coordinate is a scalar section mismatch, the gray horizontal
line is its zero level, and only the horizontal locations of the marked
crossings are being compared.  Panel~\textbf{(b)}
...
\(D_3^{\rm fin}\).  The green same-delay control is exactly zero because its
two perturbation terms cancel.  Panel~\textbf{(c)} repeats the quotient for the explicit
growing family in \cref{prop:rw-growing-families}.  The computed values for
\(N=3,5,9,17,33\) lie within \(0.008\) of the common exact coefficient
\(-1/2\), illustrating that the normalized response does not decay with
network size in this uniformly mixing family.
