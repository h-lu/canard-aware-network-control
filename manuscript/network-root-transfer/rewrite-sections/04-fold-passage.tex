...
Assume the hypotheses of \cref{thm:rw-first-moment-map} and the uniform
base-layer bound \eqref{eq:rw-base-layer-bound}.  Fix a compact interval
\(I_\nu\) and \(\eta_*>0\).  Uniformly for \(\nu\in I_\nu\) and
\(\norm{\eta}_{\mathfrak T_N}\le\eta_*\), and for all sufficiently small
\(\delta\), there is a two-dimensional locally invariant manifold in the
RFDE phase space, represented by an injective graph of compatible histories
over \(\mathcal U_\delta(C_0)\),
\[
 h=H_{N,\delta,\nu,\eta}(X,Y).
\]
...
particular,
\(D_\eta q_{1,N}=0\).  Present evaluation is a left inverse of the history
lift, so every retained graph member is an actual RFDE history and the lift
is injective.
\end{proposition}

\begin{proof}
Here \(\Phi_Q^s\) denotes the flow of the planar field \(Q\),
\(\mathcal I_{Q,H}\) is the associated history lift, and \(G_N\) is the
...
Let \(R=(R_0,\ldots,R_m)\in\mathfrak T_N^{\rm red}\) be an admissible
...
Choose two one-sided traces on the locally invariant manifold, impose the phase
\(X(0)=0\), and place the endpoints on the same regular level of the first
integral of \(q_0\).  Denote the resulting scalar matching function by
\(D_N^{\rm fin}(\delta,\nu,\eta)\).  If \(z^a\) and \(z^r\) are the
attracting and repelling traces at the phase section, its normalization is
\begin{equation}
 D_N^{\rm fin}
 =\frac{2}{\alpha e}
...
\end{equation}
The traces are exact histories on the retained interval.  Outside that
...
On a common parameter neighborhood, independently of \(N\),
\begin{align}
...
\((\mathfrak T_N^{\rm red})^*\).  These estimates do
