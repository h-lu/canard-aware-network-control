...
order zero on the same diagonal.  Its causal-entry restriction is now
uniform in the stronger graph norm controlling every prefix flight-time
action.  On this global source class the weighted-speed term already
controls those recorded prefixes; their independent necessity is not
...
derivatives on a reserved (O(S_\delta)) terminal window.  An exact slow
terminal-clock pullback now converts the variable-duration RFDE to the old
fixed interval, with uniformly controlled delay feet and an explicit
continuous-terminal-source range.  Its full physical-event border is
uniformly invertible in the clock-aligned pullback norm
`||v-A_chi beta||+|beta|/rho`; the old weighted hybrid product norm is not
uniformly equivalent and is not claimed to be.  A second local contraction
in that aligned chart constructs one literal original-endpoint RFDE segment
at every fixed admissible parameter pair, including the actual speed/history
duration columns.  Its terminal slow jet is re-proved with the coarser
`b_(delta,kappa)` state ledger and the explicit flushing limit
`E_jet b rho/delta^2 -> 0`, rather than inferred from the fixed-envelope
`lambda_delta` ledger.
The action-weighted parameter/gauge family of these endpoint segments,
past-complete branch histories, terminal-to-terminal relation, and physical
root remain open.
...
- [x] Prove the fixed-reference causal-entry phase--normal inverse in the
...
- [x] Prove the structured post-flushing terminal tangent theorem for that
      ...
      slow-jet size subcondition on the reserved clock buffer.
- [x] Prove the exact slow terminal-clock pullback and fixed-parameter
      moving-duration phase--normal endpoint theorem: fully typed upper-block
      commutator, explicit continuous terminal-source range, uniform first
      and second normalized foot derivatives, clock-aligned rather than raw
      hybrid domain/range norms, local nonlinear contraction, literal
      original endpoint, actual speed/history duration columns, and a
      separate `b_(delta,kappa)` terminal-jet flushing ledger on both
      branches.
- [ ] Prove the action-weighted parameter/gauge jets of the moving-duration
      original-endpoint segments before the terminal-to-terminal nonlinear
      outer tracker.  The pointwise strong-space theorem does not supply
      these jets or past-complete endpoint histories.
...
moving RFDE BVP by itself.  The fixed-reference order-zero normal/collar inverse
is now diagonal-uniform, and its causal-entry block is uniformly invertible
...
on the admissible wedge.  The exact slow terminal clock then closes the
fixed-parameter moving-duration extension of that split-range core without a
right-hand envelope, including its complete speed/history duration columns.
The next valid construction is its action-weighted parameter/gauge family,
...
This does not compare different reference charts or the full gauge-dependent
tracker domains.  The scalar terminal Schur row,
...
closed; the fixed-parameter moving-event residual/inverse and physical
phase--normal terminal estimate are also closed by the slow clock.  The
original-endpoint action-weighted parameter theorem, terminal-to-terminal nonlinear tracker
...
This fixed-parameter endpoint closure does not close Issue #32 and does not
