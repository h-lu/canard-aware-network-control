...
construct the invariant graph as a graph of compatible RFDE histories, then
solve the two finite-interval trace problems, and finally compare their
...
\(d_*>0\) may be chosen so that the retained fold neighborhood and all points obtained
...
continues to enter only through \(B_{k,N}+E_k\).

The product estimate following \cref{eq:fold-poisson-bound}, the uniform
layer bound, and the fixed number of delay atoms give a common majorant
\begin{equation}
 A_\delta=P(R)e^{cR}
...
fixed and all parameter dependence remains in the layer matrices.

For a complete planar field \(Q\) and a map \(H:\mathbb R^2\to E_N\),
define the history lift and the finite evaluation tuple by
\begin{align}
...
\subsection{The invariant graph and its finite jets}
...
\(\mathfrak T_N^{\rm red}\).  The flow variational equations and the
...
belongs to an earlier block.  A product containing two positive-order jets
...
\(\mathcal K^{-1}\), gives on the retained fold neighborhood where the
...
\end{equation}
in the prolonged weighted norm.  Split
...
\end{equation}
The same estimate holds on the retained normal strip
...
is
\(K\pi_N^T\sum_kE_k\mathbf1(X_0-X_k)\), which vanishes because
\(\pi_N^TE_k=0\) for every \(k\).  The base flow and the zeroth graph are
...
translations, so their values in the retained fold neighborhood use only the original
equations, not the global extension.

\subsection{Compatible and injective RFDE histories}
...
Present evaluation is a left inverse:
\begin{equation}
 \pi_{\mathbb R^2}\mathcal I_{Q,H}(u)(0)=u.
 \label{eq:supp-present-left-inverse}
\end{equation}
Hence the history lift is injective.  To see the same fact in the original
...
\end{equation}
Thus equality of two lifted histories is equality in the full RFDE state,
...
\subsection{The two one-sided trace problems}

The finite-interval calculation needs two planar traces that agree with the
invariant graph on \([-S,S]\).  Extend its reduced field from the retained
...
\begin{equation}
 E(s)=e^{s^2/2},\qquad I(s)=\int_0^sE(t)\,\dd t,\qquad
 \tau(s)=(-1,s)^T,\qquad
 n(s)=(I(s),E(s)-sI(s))^T.
...
The estimates
\begin{equation}
 \abs{I(s)}\le CE(s)\langle s\rangle^{-1},\qquad
 \abs{E(s)-sI(s)}\le CE(s)
...
\(I_\nu\times\mathfrak T_N^{\rm red}\), the matching function has the
exact decomposition
\begin{align}
 D_N^{\rm fin}
 ={}&\delta\sqrt{2\pi}(\nu-\nu_{0,N})
