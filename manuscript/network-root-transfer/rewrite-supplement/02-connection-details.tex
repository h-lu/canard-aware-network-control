...
If \(\Sigma\) is a regular event section, then
\(\mathscr H_\Sigma^2\) denotes the compatible \(C^2\) history section and
\(\mathcal P_{\Sigma_1\Sigma_2,p}\) denotes the forward event map between
two such sections.  The event map is used only where the crossing speed has
...
the finitely many losses caused by history traces, event maps, and one
...
Forward continuation gives a unique transverse hit
\(A_p^0\in\mathscr H_{\Sigma_0}^2\).  On each fixed positive regular
...
\begin{equation}
 A_\pm(\lambda_t)
 =J_\pm+\delta^2K\sum_{k=0}^m(B_{k,N}+E_{k,N})
...
\end{equation}
where \(C_A\) is independent of \(N\), \(\delta\), and the admitted
...
nonzero crossing speed.  The RFDE invariant-manifold theorem and the event
implicit-function theorem then give the regularity asserted in
...
once they satisfy the same RFDE and use the same event clock, obey a forward
...
Let \(\mathcal I_p\) be the history lift associated with the invariant graph in
\cref{prop:fold-invariant-histories}, and let \(e_{\rm pl}\) denote present
evaluation of the two fold variables.  Let
...
\begin{lemma}[Stable-manifold graph on a common event section]
\label[lemma]{lem:supp-intermediate-section-graphs}
Assume \(\mathrm{(H_{\rm glob})}\).  Let \(\Sigma_b\) be the event section
...
event section.  Its domain contains a ball of radius \(Rb_\delta\), with
...
tangent vector normal to the event section, then
\begin{equation}
...
stable manifold and the event implicit-function theorem give a local
...
\(O(b_\delta)\) in \cref{eq:supp-fading-history-norm}.  Applying the event
map and differentiating its defining equation gives
...
\end{equation}
The entry row is equality in the compatible history space.  The event row
...
variation-of-constants formula, and the event row therefore make the
...
It remains to evaluate the event-normal tangent vector at \(\Sigma_0\).  For
the singular fold equation
the independent solution normalized by the first integral is
\[
 n(s)=\binom{I(s)}{E(s)-sI(s)},\qquad
 E(s)=e^{s^2/2},\qquad I(s)=\int_0^sE(\zeta)\,\dd\zeta.
\]
Intersecting with a fixed-\(r\) event section removes the time component
\(I(s)\) and leaves
\(E(s)\) in the event-normal direction.  In the variables of
\cref{eq:rw-fold-coordinates}, the recovery component is
\(-\delta^2E(s)\{1+o(1)\}\).  Division by
\[
 w_{0,N}'(-\delta s_b/(2\alpha))
 =\delta s_b\{1+o(1)\}
\]
after normalization by the recovery-coordinate derivative gives
\(a_{0b,p}\asymp\delta E_b/s_b\).  The fold graph bounds, the
boundary-value inverse in \cref{lem:supp-central-boundary-inverse}, and the
\(C^1\) event map give the logarithmic derivative estimate in
...
neighborhood and preserve the event-speed margin.
...
membership functions back to \(\Sigma_0\), without inverting a history
event map, gives
\begin{equation}
 \norm{\mathbf D_p^1
