...
| Diagonal fixed-base phase--normal inverse | `cor:diagonal-raw-compatible-phase-normal-inverse`, `lem:current-absorbed-delay-splitting`, `lem:repelling-component-generations` | On $R_\delta=\varkappa(\delta S_\delta^3)^{1/2}$ the raw-compatible fixed-reference order-zero phase--normal graph inverse is uniform in $N,\delta$. Separately, on the attracting outer buffer the evolution kernel is controlled through one endpoint derivative with rate $\exp\{-cR_\delta\Delta s/\delta\}$, while one zeroth-order positive-delay Duhamel generation is $O(\varepsilon/R_\delta)$. On a repelling outer interval $R_\delta/2\le |r_r|\le R_\delta$, only the terminal scalar kernel is proved to have the same $R_\delta/\delta$ rate scale. No repelling decorated strict-generation estimate, higher diagonal jet, arbitrary-source strong-terminal inverse, complete moving-duration column, or full terminal Schur factor is asserted |
...
- The preparation-independent critical curve, frozen-resource
  ...
  Thus the remaining terminal proof must split
  $\lambda_\delta$-scaled structured bulk from genuinely localized
  boundary/terminal columns; it cannot use the old unrestricted rough norm.
  Separately, on the attracting outer buffer the
  evolution kernel is controlled through one endpoint derivative at rate
  $cR_\delta/\delta$, while one zeroth-order positive-delay Duhamel
  generation is $O(\varepsilon/R_\delta)$.  On the repelling outer interval
  $R_\delta/2\le |r_r|\le R_\delta$, only the terminal scalar kernel is
  proved to have the same $R_\delta/\delta$ rate scale.  The next problem is
  the moving-duration source-split strong-terminal inverse and complete terminal column,
  not a literal recut or a new fixed-reference order-zero inverse.  The
