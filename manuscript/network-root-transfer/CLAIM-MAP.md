...
| Inner-anchor endpoint-recut obstruction | `prop:inner-anchor-endpoint-recut-obstruction` | An explicit admissible scalar phase source gives wrong-sided outer endpoint displacements on both branches and forces $|\widehat\tau|/\rho_\delta\to\infty$ and $(|\widehat\tau|/\delta)/S_\delta\to\infty$ at fixed $r_{\rm out}$. Hence the inner-anchored relative inverse, strict scalar speed sign, and $O(S_\delta)$ interior buffers do not imply literal original-endpoint recutting. Here $\widehat\tau$ is an algebraic bordered-row coordinate, not a constructed physical event time. This is a phase-core nonimplication theorem, not an assertion that the source is a full RFDE residual or that model-specific cancellation is impossible; the physical causal-entry/terminal-event BVP remains open |
...
- The preparation-independent critical curve, frozen-resource
  ...
  bordered fold-time scales much larger than an $O(S_\delta)$ window; the
  correct next problem is a newly centered causal-entry/terminal-event BVP,
  not a literal recut.  The relative phase-to-state estimate also
