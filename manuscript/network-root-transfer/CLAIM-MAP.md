...
- The preparation-independent critical curve, frozen-resource
  ...
  terminal history.  The next problem is therefore the
  rough-middle-to-incoming trace/prefix assembly, the structured phase
  column in that same strong norm, and the event-aligned $K$ correction;
  it is not a literal recut or a new fixed-reference order-zero inverse.  The
