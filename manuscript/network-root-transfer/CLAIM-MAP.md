...
| Speed-aligned repelling terminal row | `prop:repelling-speed-aligned-terminal-row` | Replacing the future scalar row by the exact row $\varepsilon Q_e$ preserves the fixed-phase normal isomorphism because its pure terminal-column coefficient is $1+O(R_\delta^2+\varepsilon/R_\delta+\mathcal E_{r,T})$.  At every eventual nonlinear zero of this row, exactly $Q_e=0$, $q_{*,e}/q_0(e)=1$, $K_p=R_q(e)=O(\varepsilon)$, and $\kappa_p=O(\rho_\delta/R_\delta)=o(1)$.  The proposition selects the future Green coordinate but does not construct the nonlinear zero, prove the terminal $Z_s,Z_{ss}$ gate, or produce a past orbit or physical root |
...
- The preparation-independent critical curve, frozen-resource
  ...
  $1+o(1)$ and forces $K_p=R_q=O(\varepsilon)$ at any eventual nonlinear
  zero.  The next problem is therefore nonlinear split-range membership and
  contraction, followed by the second-order terminal $Z$ jet required for
  the history shear; it is not a literal recut or a new fixed-reference
  order-zero inverse.  The
