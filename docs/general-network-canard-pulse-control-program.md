...
Status: **research contract, updated 2026-08-25**. This document defines the work
...
| Target-amplitude physical history chart | The incoming C4 rectangle is globally injective by exact \(\mathbb Q(\sqrt5)\) Bernstein positivity.  A 192-bit Picard--Bernstein proof and a separate same-kernel replay at 256-bit precision cover all 8,000 cells of \([-3,1]\times[-1/20,1/20]\), including the delayed C4 patch, and establish the physical P-matrix inequalities and early \(X_t<0\) | Cover the second method step \((1,3]\), prove the late cross-separation inequality, enlarge the collar, and compose the two strips into the global embedded chart before invoking the graph and trace/Fredholm machinery |
...
| Leaky-recovery autonomous bistable slice | The quiet equilibrium is locally exponentially stable for arbitrary positive delays.  Independently audited directed radii theorems validate distinct inner and outer phase-fixed periodic RFDE orbits and their bordered inverses.  Before either delay returns, the one-unit pulse generates an exact jointly smooth, injective and positively oriented complete-history curve.  A non-directed three-mesh shooting artifact locates a stable-separator target near \(J=0.301135337086902\) with scaled derivative near \(-3.449\) | Prove the outer zero and inner one nontranslation unstable Floquet indices; validate the unstable covector and local stable manifold; certify the pulse intersection, two routed sides and endpoint basin inclusions; then continue the orbit and onset sensitivities on one common \((a,\kappa_3)\) box |
