...
Status: **research contract, updated 2026-08-26**. This document defines the work
...
The legacy FHN 319-cell unit-circle and 32,046-leaf right-half Floquet sources
use the physically correct unshifted-coefficient/output-phase form.  Their
frozen manifests validate, and a separate four-test operator oracle verifies
the equivalent shifted/input form and rejects both mixed conventions; see the
[representation audit](floquet-input-mode-phase-integrity-audit.md).  Their
original theorem status is retained.  The
leaky orbit, basin, response and transverse results are tracked separately.
...
42-page paper would hide independent hypotheses and overstate what it proves.
...
| Quadratic period-locked common RFDE | Exact carrier algebra, center periodic-orbit and output invariance, qualitative three-parameter periodic IFT, the fixed-scaled-support scalar root law, and its dimension-uniform canonical zero-graph lift to every finite topology with a common Dobrushin gap are proved. The linear carrier's leading pairing is exactly zero. At the exact center gain pair, a 32,046-leaf replay proves local full-network orbital attraction for \(|\eta|\le3\times10^{-6}\) and every fixed finite \(\tau(Q)\le1/4\) topology; the independent representation oracle confirms the stored convention | Validate the complete-history root and dynamic-adjoint ratio at \(\varepsilon=1/5\), then prove an input-independent root/event comparison; a joint gain--eta box, arbitrary histories, a network-uniform nonlinear basin, closing-gap families, and basin uniqueness remain outside the theorem |
| Fixed-\(\varepsilon\) quadratic root gate | The exact blueprint specifies the enlarged-history two-sided BVP, jump complement, advanced adjoint and radii polynomial. A later binary64 two-branch solve closes both flights, phase and the nodewise full-history jump; its endpoint-compatible entry, discrete adjoint and direct tangent give finest \(\rho_N=-0.3463310348\), with second-order mesh behavior. Exact audits prove that scalar \(G=0\) does not select a history. A nonstationary invariant history chart is locally the sliding-window trace of one trajectory segment; conversely, an independently selected complete orbit with a monotone affine phase generates the required chart. A second correction proves that continuous piecewise polynomials are conforming in \(W^{1,p}\), endpoint compatibility is not a continuous \(W^{1,p}\) row, and the old \(W^{2,p}\to L^p\) principal scale is compact rather than Fredholm | At the frozen target cutoff, separately validate the special-flow graph family and the canonically prepared planar trace field. Derive the finite-window regularized-gap row \(A_{S,\mathcal P}\nu+B_{S,\mathcal P}\) from the actual linearized prepared BVP, enclose \(A_{S,\mathcal P}\ne0\), and only then continue to \(\delta=1/\sqrt5\). Generate attracting/repelling histories through the common graph embedding, validate a terminal-local \(Y\)-phase patch (or multiple patches/time), parameter jets, and the actual trace-pair \(1/1\) intersection/cokernel hypothesis. The natural full-history \(W^{1,p}\) ledger is again \(775\)-by-\(774\), without derivative seams or compatibility rows, but it remains arithmetic until a uniform inverse, full dual-space adjoint and tails are certified. Root-section spread \(0.10229\), response spread \(0.00607\), connection-dependent endpoints and \(1.39709\) units of artificial old tail forbid promotion |
| Target-amplitude physical history chart | The incoming C4 rectangle is globally injective by exact \(\mathbb Q(\sqrt5)\) Bernstein positivity.  Two 8,000-cell, 192-bit Picard--Bernstein proofs, each with a same-kernel 256-bit replay, cover \([-3,3]\times[-1/20,1/20]\).  Their composition proves the full physical P-matrix inequalities and both cross-separation conditions | Enlarge the label collar, glue the exact C4-history and physical strips on that collar, and prove the global embedded chart before invoking the graph and trace/Fredholm machinery |
...
| General selected-event return theory | For every finite-dimensional fixed-delay \(C^k\) RFDE, a common selected event window beginning after \(k\tau_{\max}\), together with a \(C^k\) initial chart, common solution/event tube, strict endpoint signs, positive speed, and terminal-chart containment, gives a \(C^k\) moving complete-history section return. No no-earlier-hit premise is needed for selected-branch smoothness. A direct bounded-positive-time selected return identifies the intrinsic periodic-orbit stable-set germ under explicit invariant phase-isolation, common intervening-flow, retained-domain, and recurrent-hit hypotheses, without first proving \(Q=P^m\) | The threshold is sufficient, not asserted sharp. State-dependent, neutral, infinite-delay, infinite-network and infinite-dimensional node-state extensions remain outside the theorem; concrete tubes and derivative constants must still be validated model by model |
| Leaky-recovery autonomous bistable slice | The quiet equilibrium is locally exponentially stable for arbitrary positive delays.  Directed radii theorems validate distinct inner and outer phase-fixed periodic branches, bordered inverses and simple extrema on the common box \(|a-1/4|,|\kappa_3-1/200|\le10^{-10}\).  At the center, the closed-right-half-plane inner count is exactly two: the simple translation value and one simple positive nontranslation value, with zero complementary keyhole count; hence the inner orbit has exactly one unstable multiplier there.  Classical RFDE theory and exact reduced-history factorization consequently give a qualitative \(C^1\), codimension-one stable manifold in full and reduced history spaces.  The directed D4 response certificate has status ACCEPT on the common box.  The pulse curve is exact and oriented; a large Razumikhin basin and directed \(J=0.30\) complete-history capture close the quiet-side anchor.  At \(J=0.32\), a separate directed calculation now puts the full ambient history within \(2.637078616900037\times10^{-5}\) of the exact outer phase-zero history, strictly inside the ambient radius-\(10^{-4}\) ball; it is not yet capture.  The old scalar inner-graph majorant is rejected, while a six-block three-mesh pilot closes the structured \(2\times2\) majorant with Perron value \(0.04913\); the continuous-history block enclosure is still open.  A direct outer-return contract likewise gives \(\|P\|_\infty\approx0.127\) and \(\|P^2\|_\infty\approx0.00278\) on four finite sections, but no directed \(C^0\)-operator norm.  Every admitted finite Dobrushin network has rate-\(1/10\) transverse decay and a complete-line Green inverse of norm at most ten, yielding conditional exact scalar-root transfer.  The same constants prove nonlinear node-diameter decay in the validated strip, and the collective mean has the uniform quadratic defect bounds \(|R_{\rm coll}(t)|\le(703/200)\mathcal H_M(t)^2\), with the sharper componentwise delayed formula, and accumulated size \(\le(703/40+27\sqrt5/800)M_0^2\le(56483/3200)M_0^2\) | Enclose the outer signed matrix-measure return kernel, nonlinear phase chart and second variation to convert the \(J=0.32\) ambient attachment into capture; enclose the inner stable-output/\(uu\)-input return-Hessian block first, then the other five blocks and the split return ball; prove the pulse/stable-sheet crossing, two routed exits, \(J_c\), its parameter derivatives and the three-output safety ball; continue the inner index to the common box if needed for the parameter theorem; prove a strip-invariant scalar routing tube for a topology-uniform asynchronous threshold neighborhood and compare the scalar leaky canard root with physical onset.  The global outer zero index remains open but is no longer a prerequisite for the direct local-capture route |
...
| Frequency--amplitude--safety conditioning | Periodic/event adjoints, D1 continuation, D3 unique extrema, D4 response enclosures and the associated microscopic target balls were directed-certified independently of the later unstable-index count.  The transverse Halanay and oscillation-norm estimates are proved, and the legacy 32,046-leaf FHN right-half tree composes with them to give local attraction for each fixed admitted topology.  The stored unshifted/output convention is independently cross-checked against the equivalent shifted/input form | Prove a same-model trajectory connection from onset into the local periodic basin, a joint gain--eta box, weaker/closing-gap topology results, a network-uniform nonlinear basin, autonomous biological capture, and robust hardware containment; second sensitivities remain optional for larger boxes |
...
then replaces the overbuilt history-chart PDE by one independently selected
orbit, proves the correct first-order continuous scale, and gives the exact
conditional trace-pair index-\(-1\) reduction; it does not promote the
candidate to a selected root.  The
