...
source-split rough-middle/strong-terminal inverse, the complete moving-duration normal
column, the full RFDE terminal Schur factor, and the physical root remain
open.**
...
have the same `r_out/delta` rate scale.  Higher unweighted jets and the moving terminal column
still require a new calculus.  That theorem must track at least
...
Only after an endpoint-centered duration chart is fixed can `C_ter` denote
the complete bulk/collar/terminal column generated by `hat_tau`.  Let
`ell_ter` be the terminal event differential.  The scalar endpoint theorem
gives the direct coefficient
`c_ter=q_E(e_1)/q_0(e_1) in [1/2,3/2]`.  The missing estimate is the
off-diagonal correction created after the normal equations, collar, and
terminal evaluation are coupled:

```text
| c_ter^(-1) ell_ter (L_ent^hyb)^(-1) C_ter | <= 1/2
```

in the epsilon-normalized graph/action/terminal norm.  It yields the scalar
Schur factor and inverse

```text
s_ter   = c_ter - ell_ter (L_ent^hyb)^(-1) C_ter,
hat_tau = s_ter^(-1)
          { b - ell_ter (L_ent^hyb)^(-1) f },
v       = (L_ent^hyb)^(-1) { f - C_ter hat_tau }.
```

This interface requires a hybrid rough-middle/strong-terminal
arbitrary-source inverse, an actual endpoint strong buffer with the
rectangular `J_phys` jets, and an exact chart conjugacy.  The existing
fixed-reduced-base Schur theorem, supplied terminal trace chart, and
interior-hit inverse are three reusable inputs, not this theorem itself.
...
- [ ] Moving-duration normalized phase--normal terminal Schur estimate and
      the corresponding causal-entry/terminal-event action-weighted
      parameter/gauge theorem at the original endpoints.  Construct the
      hybrid arbitrary-source rough-middle/strong-terminal inverse, split
      structured bulk data from localized endpoint columns, derive its
      complete `C_ter` column, raw-compatible endpoint histories, and the
      endpoint `J_phys` family before terminal-to-terminal nonlinear tracker
      reconstruction.
...
restriction and the exact old-rough-source loss theorem.  None of these constructs the
moving-duration RFDE column, the full terminal Schur inverse, compares
different fixed-reference charts, or compares the full reparameterized
trackers on a common chart.
