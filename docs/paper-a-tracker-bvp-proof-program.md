...
uniformly on this diagonal at order zero.  Separately, the attracting
...
rough-middle/strong-terminal inverse, the complete moving-duration normal
...
There is also an arbitrary-source obstruction that the fixed-reference
inverse does not remove.  The presently available arbitrary-source bulk
estimate gives integrated phase action only at scale
`O(r_out,delta^2)`, but
...
which is not uniformly bounded.  The moving-endpoint proof must therefore
separate structured bulk columns from localized boundary/terminal columns;
it cannot demand an `O(rho_delta)` terminal phase estimate for every
arbitrary source.
...
At the linear level, let `L_ent` be the hybrid arbitrary-source operator
containing the normal equations, raw-compatible collar, and causal-entry
...
```text
s_ter   = 1 - q_e^(-1) ell_ter L_ent^(-1) C_ter,
hat_tau = s_ter^(-1)
          { b - q_e^(-1) ell_ter L_ent^(-1) f },
v       = L_ent^(-1) { f - C_ter hat_tau }.
```
...
diagonal order-zero inverse.  None of these constructs the
