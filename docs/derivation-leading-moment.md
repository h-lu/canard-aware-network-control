...
## 6. Two-module lift and the necessary transverse functional

For the exact two-module skeleton, let \(\mathbb B(d\theta)\) be the
operator-valued row-weighted scaled-delay measure, including the module-pair
gain in each entry. With dynamical critical modes \(r_c,\ell_c\), normalized
by \(\ell_c^\top r_c=1\), define
...
Equivalently, if
\(\mathbb B_{ab}(d\theta)=B_{ab}\widehat\rho_{ab}(d\theta)\) with
\(\widehat\rho_{ab}\) a conditional probability measure, then the summand is
\((\ell_c)_aB_{ab}(r_c)_b\int\theta\,d\widehat\rho_{ab}\). The two
normalizations are not mixed.

Projecting only the unperturbed delayed translation produces the parallel
term \(M_1^{(2)}\), but the general candidate is

\[
K\Theta\longmapsto
K\left(C_\parallel M_1^{(2)}
+\mathcal J_{\perp,\delta}[\mathbb B]\right).
\tag{13}
\]

Here \(\mathcal J_{\perp,\delta}\) is generated by delayed translation into transverse
histories, inversion of the transverse RFDE range equation, and return through
the dynamic critical adjoint. Its \(\delta\)-dependence is retained because the
transverse inverse may degenerate in the canard limit. Equation (13) is **not
yet a network theorem**.
It is valid as a research target only after showing that:

1. the critical collective mode is closed at the required order;
2. the transverse functional and its \(\delta\)-scaling are calculated, or the entire Lin problem respects a layerwise critical/transverse splitting that makes the leading delay-induced term zero;
...
A sufficient closure condition for the delayed-source forcing is
...
For the leading delay-induced \(\mathcal J_{\perp,\delta}\) to vanish, the
current, instantaneous, nonlinear, and endpoint/Lin operators must also
respect the same critical/transverse splitting. The
[exact two-delay counterexample](two-module-moment-counterexample.md) keeps
...
This row-measure class is the rank-one calibration for the topology-weighted law. General rank-\(r\) modules with different operator-valued delay measures require the adjoint/transverse calculation in (12)--(13). For nonnegative receiver-self diffusion with a positive critical mode, exact layerwise closure itself collapses to common-row-measure closure by the Perron no-go lemma in the counterexample note.
