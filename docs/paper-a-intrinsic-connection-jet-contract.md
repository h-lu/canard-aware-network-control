...
diagonal.  Separately, the attracting outer-buffer evolution kernel is
...
have the same `R_delta/delta` rate scale.  The moving-duration RFDE column, full terminal Schur
inverse, original endpoint histories, repelling history sheet, physical
...
is diagonal-uniform.  Membership of the source generated by the open moving
RFDE BVP in that structured class remains unproved.  The next valid RFDE operator must release duration,
separate structured bulk sources from localized endpoint columns, and use
exactly one causal-entry row and one terminal-event row together with the
complete normal/collar column.
...
- [ ] Construct the moving-duration RFDE causal-entry/terminal-event BVP
      with its hybrid arbitrary-source inverse, structured/localized source
      split, complete normal terminal column, normalized full phase--normal
