...
- [docs/fixed-window-prepared-gap-seed.md](docs/fixed-window-prepared-gap-seed.md) -- proved finite-window linear Green row and MPFR-directed unique affine zero for a singular-hull-compatible longitudinal first-order forcing datum; its realization by one complete frozen graph preparation, positive-amplitude continuation, and the fixed-\(\varepsilon\) history root remain open;
