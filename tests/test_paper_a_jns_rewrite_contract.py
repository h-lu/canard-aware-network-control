...
def test_first_moment_and_fredholm_statements_have_separate_interfaces() -> None:
    ...
    assert "Range and a norm-optimal right inverse for the first delay moment" in model
    assert r"\label{thm:rw-structured-delay-range}" in model
    assert "It vanishes if every allowed direction preserves row sums" in model
    assert r"\label[proposition]{prop:rw-growing-families}" in model
    assert r"\Lambda_N(R_0,R_1)=-\frac{K\sigma}{2D\rho}" in model
    ...
    assert r"\label{thm:rw-local-fold-matching}" in model
    assert r"D_\eta D_N^{\rm fin}" in model
    assert "its zero set is not asserted to be a heteroclinic" in model
...
def test_active_tex_has_no_literal_qquad_typo() -> None:
    paths = [PAPER / "main.tex"]
    paths.extend(sorted((PAPER / "rewrite-sections").glob("*.tex")))
    paths.extend(sorted((PAPER / "appendices").glob("*.tex")))
    paths.extend(sorted((PAPER / "figures").glob("*.tex")))
    for path in paths:
        text = path.read_text(encoding="utf-8")
        assert re.search(r"(?<!\\)qquad", text) is None, path
        assert r"Equation \cref" not in text
        assert r"first equation in \cref" not in text
...
def test_numerical_figure_is_scoped_as_a_finite_section_diagnostic() -> None:
    ...
    fold = compact(
        source("manuscript/network-root-transfer/rewrite-sections/04-fold-passage.tex")
    )
    plot_source = source(
    ...
    assert r"is not \(D_N^{\rm fin}\)" in figure
    assert "does not compute a heteroclinic connection or a maximal canard" in figure
    assert "Status: `MIXED`" in contract
    assert "the gray horizontal line is its zero level" in fold
    assert "Figure~\\ref{fig:three-node-finite-section} is read as follows" not in introduction
    assert "Invisible redistribution moves the zero" in plot_source
    assert "Scaled shift approaches theory" in plot_source
    assert "Coefficient persists with $N$" in plot_source
    assert r"(\theta_0,\theta_1)=(1,2)" in figure
    assert r"N=3,5,9,17,33" in figure
