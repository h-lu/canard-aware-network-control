#!/usr/bin/env python3
"""Generate the fixed-epsilon selected-Fredholm structure certificate."""

from __future__ import annotations

import argparse
from hashlib import sha256
import json
from pathlib import Path
import sys


REPOSITORY = Path(__file__).resolve().parents[1]
SOURCE_ROOT = REPOSITORY / "src"
if str(SOURCE_ROOT) not in sys.path:
    sys.path.insert(0, str(SOURCE_ROOT))

from canard_control.fixed_epsilon_selected_fredholm_structure import (  # noqa: E402
    BLUEPRINT_NOTE_SHA256,
    BLUEPRINT_RESULT_SHA256,
    BLUEPRINT_SOURCE_SHA256,
    CANDIDATE_NOTE_SHA256,
    CANDIDATE_RESULT_SHA256,
    CANDIDATE_SOURCE_SHA256,
    reference_selected_fredholm_structure_payload,
    validate_selected_fredholm_structure_payload,
)


def _digest(path: Path) -> str:
    return sha256(path.read_bytes()).hexdigest()


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--output",
        type=Path,
        default=(
            REPOSITORY
            / "experiments/results/fixed_epsilon_selected_fredholm_structure.json"
        ),
    )
    arguments = parser.parse_args()
    source = (
        REPOSITORY
        / "src/canard_control/fixed_epsilon_selected_fredholm_structure.py"
    )
    generator = Path(__file__).resolve()
    upstream_paths = {
        "blueprint_source_sha256": (
            REPOSITORY / "src/canard_control/fixed_epsilon_quadratic_root_bvp.py"
        ),
        "blueprint_result_sha256": (
            REPOSITORY
            / "experiments/results/fixed_epsilon_quadratic_root_bvp.json"
        ),
        "blueprint_note_sha256": (
            REPOSITORY / "docs/fixed-epsilon-quadratic-root-bvp.md"
        ),
        "candidate_source_sha256": (
            REPOSITORY
            / "src/canard_control/fixed_epsilon_two_sided_candidate.py"
        ),
        "candidate_result_sha256": (
            REPOSITORY
            / "experiments/results/fixed_epsilon_two_sided_candidate.json"
        ),
        "candidate_note_sha256": (
            REPOSITORY / "docs/fixed-epsilon-two-sided-candidate.md"
        ),
    }
    observed = {key: _digest(path) for key, path in upstream_paths.items()}
    expected = {
        "blueprint_source_sha256": BLUEPRINT_SOURCE_SHA256,
        "blueprint_result_sha256": BLUEPRINT_RESULT_SHA256,
        "blueprint_note_sha256": BLUEPRINT_NOTE_SHA256,
        "candidate_source_sha256": CANDIDATE_SOURCE_SHA256,
        "candidate_result_sha256": CANDIDATE_RESULT_SHA256,
        "candidate_note_sha256": CANDIDATE_NOTE_SHA256,
    }
    if observed != expected:
        raise RuntimeError("an upstream fixed-epsilon package binding has drifted")
    audit = reference_selected_fredholm_structure_payload()
    validate_selected_fredholm_structure_payload(audit)
    payload = {
        "audit": audit,
        "manifest": {
            "generator": (
                "experiments/fixed_epsilon_selected_fredholm_structure.py"
            ),
            "proof_source": (
                "src/canard_control/fixed_epsilon_selected_fredholm_structure.py"
            ),
            "proof_source_sha256": _digest(source),
            "generator_sha256": _digest(generator),
            **observed,
            "arithmetic": (
                "exact integer and SymPy block algebra; no selected charts, "
                "floating-point Fredholm matrix, or interval inverse"
            ),
        },
    }
    arguments.output.parent.mkdir(parents=True, exist_ok=True)
    arguments.output.write_text(
        json.dumps(payload, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )


if __name__ == "__main__":
    main()
