#!/usr/bin/env python3
"""Generate or check the source-bound inner stable-manifold Stage-2 contract."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="validate the registered Stage-2 result without rewriting it",
    )
    ...
    payload = build_stage2_stable_manifold_result(REPOSITORY)
    validate_stage2_stable_manifold_result(payload, REPOSITORY)
